/-!
# Source package model for `p-adic-hodge-theory-canonical-lane`
...
def sourceCheckoutHead : String := "fef946a1743e97566da342357cc13eb7466bff75"
...
def sourceFiles : List SourceFile := [
  ...
  { path := "scripts/pht_closure_guard.py", sha256 := "67234aa8a4ef0494bc1b4f3e8b4ee718058b5c010355ebeac3eb87800d47e09d", functionCount := 9, classCount := 0, assignmentCount := 8, parseOk := true },
...
def sourceFunctions : List SourceFunctionDecl := [
  ...
  { file := "scripts/pht_closure_guard.py", name := "_finite", args := ["v"], returns := "bool", doc := "", line := 31, isAsync := false },
  { file := "scripts/pht_closure_guard.py", name := "_resolve", args := ["path_str"], returns := "Path", doc := "", line := 35, isAsync := false },
  { file := "scripts/pht_closure_guard.py", name := "_bootstrap_registry", args := ["path"], returns := "None", doc := "", line := 42, isAsync := false },
  { file := "scripts/pht_closure_guard.py", name := "_load_registry", args := ["path"], returns := "dict[str, Any]", doc := "", line := 48, isAsync := false },
  { file := "scripts/pht_closure_guard.py", name := "_load_sigma_from_stitch", args := ["path"], returns := "float | None", doc := "", line := 60, isAsync := false },
  { file := "scripts/pht_closure_guard.py", name := "_entry", args := ["constants", "key"], returns := "tuple[float | None, bool]", doc := "", line := 77, isAsync := false },
  { file := "scripts/pht_closure_guard.py", name := "compute_report", args := ["data", "sigma_star", "strict_coh_zero"], returns := "dict[str, Any]", doc := "", line := 88, isAsync := false },
  { file := "scripts/pht_closure_guard.py", name := "append_history", args := ["path", "report"], returns := "None", doc := "", line := 121, isAsync := false },
  { file := "scripts/pht_closure_guard.py", name := "main", args := [], returns := "None", doc := "", line := 128, isAsync := false },
...
def sourceAssignments : List SourceAssignmentDecl := [
  ...
  { file := "scripts/pht_closure_guard.py", name := "SCRIPT_DIR", value := "Path(__file__).resolve().parent", line := 13 },
  { file := "scripts/pht_closure_guard.py", name := "PROJECT_ROOT", value := "SCRIPT_DIR.parent", line := 14 },
  { file := "scripts/pht_closure_guard.py", name := "DEFAULT_REGISTRY", value := "\"artifacts/constants_registry.json\"", line := 16 },
  { file := "scripts/pht_closure_guard.py", name := "DEFAULT_STITCH", value := "\"artifacts/stitch_constants.json\"", line := 17 },
  { file := "scripts/pht_closure_guard.py", name := "DEFAULT_OUT", value := "\"repro/certificate_runtime.json\"", line := 18 },
  { file := "scripts/pht_closure_guard.py", name := "DEFAULT_HISTORY", value := "\"repro/drift_guard_runs.jsonl\"", line := 19 },
  { file := "scripts/pht_closure_guard.py", name := "PRIMARY_KEYS", value := "[\"kappa_period\", \"sigma_comparison\", \"kappa_compact\", \"rho_rigidity\", \"hodge_tate_transfer\"]", line := 21 },
  { file := "scripts/pht_closure_guard.py", name := "COHERENCE_KEY", value := "\"eps_coh\"", line := 28 },
...
def constantSpecs : List ConstantSpec := [
  { group := "constants", key := "kappa_period", status := "derived_numeric", formula := "c_core_raw * density_raw - e_core_raw", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 4 / Appendix A", validation := "required_positive", componentKeys := ["c_core_raw", "density_raw", "e_core_raw"] },
  { group := "constants", key := "sigma_comparison", status := "derived_numeric", formula := "capture_floor_raw - transport_loss_raw - restart_loss_raw", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 5 / Appendix B", validation := "required_positive", componentKeys := ["capture_floor_raw", "restart_loss_raw", "transport_loss_raw"] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 5 / Appendix C", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 6 / Appendix D", validation := "required_positive", componentKeys := ["rho_rigidity_raw"] },
  { group := "constants", key := "hodge_tate_transfer", status := "derived_numeric", formula := "c_transfer_raw * transfer_gain_raw - e_transfer_raw", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 6 / Appendix D", validation := "required_positive", componentKeys := ["c_transfer_raw", "e_transfer_raw", "transfer_gain_raw"] },
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 6 / Appendix E", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 5 / stitch constants", validation := "required_positive", componentKeys := ["sigma_star_can_raw"] }
]

def registryConstants : List RegistryConstant := [
  { key := "eps_coh", value := "0.0", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#eps_coh", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 6 / Appendix E" },
  { key := "hodge_tate_transfer", value := "1.02951", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#hodge_tate_transfer", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 6 / Appendix D" },
  { key := "kappa_compact", value := "0.8038585209003215", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_compact", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 5 / Appendix C" },
  { key := "kappa_local", value := "1.0940320000000001", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_local", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 4.5 / Appendix A" },
  { key := "kappa_period", value := "1.091838", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_period", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 4 / Appendix A" },
  { key := "packet_local", value := "1.0315400000000001", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#packet_local", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 6.2 / Appendix E.4" },
  { key := "rho_rigidity", value := "1.077", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#rho_rigidity", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 6 / Appendix D" },
  { key := "sigma_comparison", value := "1.075", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sigma_comparison", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 5 / Appendix B" },
  { key := "sigma_matching", value := "1.075", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sigma_matching", sourceSection := "paper/P_ADIC_HODGE_THEORY_PREPRINT.md Section 5.2 / Appendix B" }
]
